...
async function requestAIDraft(prospect) {
  const response = await fetch(`${TABLESIDE_AI_URL}/api/generate-draft`, {
    ...
    body: JSON.stringify({businessName:prospect.business, city:prospect.city, website:prospect.website, instagram:prospect.instagram, notes:prospect.notes})
  });
  let data = {};
...
window.requestAIDraft = requestAIDraft;

function getAppState() {
  if (typeof window.getTableSideState === 'function') return window.getTableSideState();
  throw new Error('App state interface unavailable.');
}

function putAppState(next) {
  if (typeof window.replaceTableSideState === 'function') return window.replaceTableSideState(next);
  throw new Error('App state interface unavailable.');
}
...
async function generateAIApproval(id) {
  ...
  if (box) { box.disabled = true; box.value = 'Generating personalized AI draft…'; }
  try {
    const draft = await requestAIDraft(prospect);
    const latest = getAppState();
    latest.approvals ||= []; latest.activity ||= [];
    const approval = {id:Date.now(), prospectId:id, action:'AI-drafted outbound outreach', status:'Pending', subject:draft.subject, body:draft.body, package:`TABLESIDE GROWTH — AI OUTREACH DRAFT\n\nBusiness: ${prospect.business}\n\nSUBJECT\n${draft.subject}\n\nCUSTOMER-FACING DRAFT\n${draft.body}\n\nAUTHORIZATION\nNot sent. Explicit human approval is required before any communication leaves TableSide Growth.`};
    latest.approvals.unshift(approval);
    latest.selected = id; latest.selectedApproval = approval.id;
  ...
  } finally { if (box) box.disabled = false; }
}
