...
async function sendApprovedEmail(to, subject, body) {
  if (!gmailIsConnected()) {
  ...
  if (!to || !subject || !body) {
    alert('Email is missing a recipient, subject, or body.');
    return false;
  }
  ...
  try {
    await gapi.client.gmail.users.messages.send({
      userId: 'me',
      resource: { raw: encodeBase64Url(message) }
    });
...
async function testGmail() {
  ...
  try {
    ...
    alert('Gmail test failed. Check the browser console.');
  }
}

function connectGmail() {
  if (!gmailGapiReady || !gmailGisReady) {
    alert('Google services are still loading. Try again in a few seconds.');
    return;
  }

  gmailTokenClient.callback = response => {
    if (response.error) {
      console.error('Gmail connection failed:', response);
